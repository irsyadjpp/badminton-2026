...
// Mock Data (in a real app, this comes from getPlayerSession())
const MOCK_PLAYER_DATA = {
    email: "atlet.dev@gmail.com",
    password: "password123", // Password should not be pre-filled
    nik: "3273012345678901",
    fullName: "Atlet Development",
    birthDate: "1998-05-20",
    gender: "Laki-laki",
    phone: "081234567890",
    address: "Jl. Badminton No. 1, Bandung",
    nickname: "Atlet Dev",
    jerseySize: "L",
};
...
export default function PlayerProfilePage() {
  ...
  const form = useForm<PlayerRegisterValues>({
    ...
    defaultValues: MOCK_PLAYER_DATA,
  });
  ...
  const onSubmit = async (data: PlayerRegisterValues) => {
    ...
    if (res.success) {
      toast({ title: "Profil Diperbarui!", description: "Data Anda berhasil disimpan.", className: "bg-green-600 text-white" });
    } else {
      ...
      <Card className="w-full max-w-lg border-zinc-800 bg-zinc-900 text-white">
        <CardHeader>
            <div className="flex items-center gap-4 mb-2">
                <Button asChild variant="outline" size="icon" className="border-zinc-700">
                    <Link href="/player/dashboard"><ArrowLeft className="w-4 h-4" /></Link>
                </Button>
                <div>
                    <CardTitle className="text-2xl font-black text-primary uppercase">Edit Profil Atlet</CardTitle>
                    <CardDescription>Pastikan data Anda selalu valid dan terkini.</CardDescription>
                </div>
            </div>
        </CardHeader>
        <CardContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField control={form.control} name="email" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input {...field} readOnly className="bg-black border-zinc-800 text-zinc-400" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="password" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="password" placeholder="Isi untuk ganti password" {...field} className="bg-black border-zinc-700" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
              ...
              <div className="p-4 border border-zinc-700 rounded-lg bg-zinc-950/50 space-y-4">
                 <div className="flex items-center gap-2 text-xs text-green-500 font-bold uppercase mb-2">
                    <ShieldCheck className="w-4 h-4"/> Data Terenkripsi
                 </div>
                 <FormField control={form.control} name="nik" render={({ field }) => (
                    <FormItem>
                        <FormLabel>NIK (KTP/KK)</FormLabel>
                        <FormControl><Input {...field} readOnly className="bg-black border-zinc-800 text-zinc-400" /></FormControl>
                        <FormMessage />
                    </FormItem>
                 )} />
